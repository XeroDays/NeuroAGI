...
│   │   ├── app.js            # Home screen: populates UI, reads the Reasoning level dropdown (#select-reasoning) and stashes the chosen value in sessionStorage['neuroagi:reasoningLevel'], awaits resetUsageTotals() to zero the cost + tokens bubbles for a fresh diagnostic run. On submit, if the issue text is non-empty it runs the QueryEnhancer: shows the circular spinner on the send button (.is-loading), ensures #qe-toast-stack exists, subscribes to onQueryEnhancerProgress (routes typed events to createWorkerProgressPanel({ position:'bottom-right', attachToSelector:'#qe-toast-stack' }) and { message, status } to showToast), awaits enhanceQuery({issue,gender,age}), calls progressPanel.hide() before navigation, and uses the returned enhancedQuery string (falls back to the raw issue on failure/empty) as the issue URL param when navigating to the questionnaire. Empty issue skips the enhancer
...
│   ├── styles/
│   │   ├── app.css           # Home screen pastel theme + .qe-toast-stack bottom-right text toasts + .qe-toast-stack .q-worker-snack slide-in-from-right override
