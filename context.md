...
└── shared/
    └── ipc/
        └── channels.js       # IPC channel name constants (mirrored in preload). All channels: PING, ENHANCE_QUERY, QUERY_ENHANCER_PROGRESS, START_REPORT_COLLECTION, SUBMIT_QUESTIONNAIRE, GOTO_LABORATORY, SUBMIT_LABORATORY, GOTO_PRE_DOCTOR_ROOM, SUBMIT_PRE_DOCTOR_ROOM, START_DOCTOR, DOCTOR_STREAM_DELTA, DOCTOR_STREAM_REASONING_DELTA, DOCTOR_STREAM_DONE, DOCTOR_STREAM_ERROR, GET_USAGE_TOTALS, RESET_USAGE_TOTALS, USAGE_UPDATE, OPEN_DEV_TOOLS, GET_MODELS_CONFIG, UPDATE_MODELS_CONFIG
```
...
4. **Stage 2 — lookup**: for each detected medicine (sequentially so each gets its own toast) it emits `Looking up <name>…` and calls `webSearch.search('active ingredient and generic formula of the medication "<name>"', { includeAnswer: true, maxResults: 3 })` (Tavily). A per-medicine lookup failure is tolerated (that medicine just gets an empty evidence set)
5. **Stage 3 — rewrite**: emits `Enhancing your query with medication details…`, builds `GenerateEnhancedQueryLLMQuery({ issue, medicines, searchResults })`, calls `AskMasterAgi(prompt, JSON_LLM_OPTIONS)`, and cleans the text via `stripFences()` (the output is plain text, not JSON). If the call fails or returns empty it **falls back** to `buildFallbackEnhancedQuery(issue, medicines)` (original issue + a `Medication by user:` section built from the detected names/mg/timing). Emits `Found N medication(s)` and returns `{ ok: true, enhancedQuery }`
...
4. The middleware builds the initial intake prompt via `GenerateQuestionnaireLLMQuery()`. It then runs a **two-stage AGI pipeline** instead of a single LLM call:
   - **Stage 1 — Fanout**: `AskAllWorkerAgis(prompt)` in `agi-service.js` issues `chatCompletion(messages, model)` calls in parallel (`Promise.allSettled`) to every enabled model from `getActiveModelIds()`. Each result is shaped uniformly: `{ model, ok, content?, error? }` — a single worker 429/network/HTTP failure doesn't abort the others.
