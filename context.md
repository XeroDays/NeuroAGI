...
│   ├── ipc/
│   │   └── register.js       # Live IPC: ping, advanceSend / advanceCancel (advance-middleware), getUsageTotals, resetUsageTotals, openDevTools, getModelsConfig, updateModelsConfig, getLogs, clearLogs. Dormant but still registered (obsolete screens): enhanceQuery, startReportCollection, skipFanoutWait, submitQuestionnaire, gotoLaboratory, submitLaboratory, gotoPreDoctorRoom, submitPreDoctorRoom, startDoctor. USAGE_UPDATE / LOG_UPDATE / ADVANCE_PROGRESS are broadcast, not handle-invoked
│   ├── middlewares/
...
│   │   ├── advance-middleware.js   # SendAdvanceChat({ messages, resume?, reasoningLevel }, sender) — starred-master chat turn for the Advance screen. CancelAdvanceChat(sender) aborts the in-flight turn
...
├── renderer/
│   ├── index.html            # Home screen (links app.css). `#models-overlay` + `#error-overlay` both use shared `.glass-overlay` backdrop. Submit navigates to Advance
│   ├── screens/
│   │   ├── advance/
│   │   │   └── index.html    # Advance chat screen (shown after home submit, or via the Home Advance button). Thread + composer; no settings overlay
│   │   ├── questionnaire/
│   │   │   └── index.html    # OBSOLETE — no longer reachable; Home now routes to Advance. Files kept for reversibility
│   │   ├── laboratory/
│   │   │   └── index.html    # OBSOLETE — no longer reachable. Files kept for reversibility
│   │   ├── pre-doctor-room/
│   │   │   └── index.html    # OBSOLETE — no longer reachable. Files kept for reversibility
│   │   └── doctor/
│   │       └── index.html    # OBSOLETE — no longer reachable. Files kept for reversibility
│   ├── scripts/
│   │   ├── constants.js      # APP_TITLE, SCREEN_QUESTIONNAIRE, SCREEN_LABORATORY, SCREEN_PRE_DOCTOR_ROOM, SCREEN_DOCTOR, SCREEN_ADVANCE, labels
│   │   ├── app.js            # Home screen: submit wired through `handleStartDiagnostics()` (send button + Ctrl/Cmd+Enter shortcut). Empty issue is a no-op. First guard: `isMasterModelSelected()` via `getModelsConfig()` — if no persisted `isMaster`, shows `#error-overlay` and aborts. Then reads Reasoning level dropdown (#select-reasoning) into sessionStorage['neuroagi:advanceReasoningLevel'], awaits resetUsageTotals(), navigates to `screens/advance/index.html?issue=…&gender=…&age=…`. Does NOT run QueryEnhancer. `#btn-advance` still opens a blank Advance chat without auto-submit
...
│   │   ├── questionnaire.js  # OBSOLETE — no longer reachable. Banner comment at top of file
│   │   ├── laboratory.js     # OBSOLETE — no longer reachable. Banner comment at top of file
│   │   ├── pre-doctor-room.js # OBSOLETE — no longer reachable. Banner comment at top of file
│   │   ├── doctor.js         # OBSOLETE — no longer reachable. Banner comment at top of file
│   │   ├── advance.js        # Advance chat: reads sessionStorage['neuroagi:advanceReasoningLevel'] (default medium) and forwards it on every advanceSend. On load, if URL has `issue`, composes first message (issue + "Patient: {age}-year-old {gender}.") and auto-runs handleSend(). No Settings chip / overlay / reasoning select
│   │   ├── advance-questions.js # Advance ask_user form renderer (text / single_select / multi_select / slider / range) used when the master model pauses for intake
...
│   ├── styles/
│   │   ├── app.css           # Home screen pastel theme + shared `.glass-overlay` backdrop (Models + error popups) + `.error-modal` compact dialog
│   │   ├── advance.css       # Advance chat pastel theme: thread, bubbles, composer, status steps, ask_user form. Settings overlay / reasoning select styles removed — Home owns reasoning
│   │   ├── worker-snack.css  # Shared .q-worker-stack / .q-worker-snack pill styles. No longer linked by home. Still used by the obsolete fanout screens
│   │   ├── questionnaire.css # OBSOLETE screens — Questionnaire + Laboratory + Pre-doctor Room pastel theme
│   │   ├── doctor.css        # OBSOLETE screen — Doctor pastel theme + tabs + thinking bubble + reasoning panel
...
└── shared/
    └── ipc/
        └── channels.js       # IPC channel name constants (mirrored in preload). Live: PING, ADVANCE_SEND, ADVANCE_PROGRESS, ADVANCE_CANCEL, GET_USAGE_TOTALS, RESET_USAGE_TOTALS, USAGE_UPDATE, OPEN_DEV_TOOLS, GET_MODELS_CONFIG, UPDATE_MODELS_CONFIG, GET_LOGS, CLEAR_LOGS, LOG_UPDATE. Dormant (handlers still registered, nothing reachable invokes them): ENHANCE_QUERY, QUERY_ENHANCER_PROGRESS, AGI_FANOUT_PROGRESS, SKIP_FANOUT_WAIT, START_REPORT_COLLECTION, SUBMIT_QUESTIONNAIRE, GOTO_LABORATORY, SUBMIT_LABORATORY, GOTO_PRE_DOCTOR_ROOM, SUBMIT_PRE_DOCTOR_ROOM, START_DOCTOR, DOCTOR_STREAM_*
```
...
2. User types health issue in text input, selects gender, age, and **Reasoning level** (`#select-reasoning`, default `medium`)
...
4. Empty issue is a no-op (the Home **Advance** button still opens a blank chat). Non-empty issue continues
5. **Master model guard:** `handleStartDiagnostics()` calls `isMasterModelSelected()` → `getModelsConfig()` → checks persisted `isMaster` on any catalog entry. If none starred, shows `#error-overlay` ("Master model required") with **OK** and **Open Models**. Flow aborts — no `resetUsageTotals()`, no navigation. Validates persisted state (post-Update), not unsaved Models popup edits
6. `app.js` stashes the Reasoning level in `sessionStorage['neuroagi:advanceReasoningLevel']` so Advance can read it after the page navigation. Advance's `getReasoningLevel()` reads this key (validates against `none|low|medium|high|very_high`, defaults to `medium` if missing) and forwards it on every `advanceSend` payload. There is no Settings overlay or reasoning dropdown on the Advance page
7. `app.js` awaits `window.electronAPI.resetUsageTotals()` so the top-right cost + tokens bubbles reset to `USD 0` / `0 tokens` before the new run begins — returning home via Back does **not** reset; only the Start button (or Ctrl/Cmd+Enter) does
8. `app.js` navigates to `screens/advance/index.html?issue=…&gender=…&age=…`. No QueryEnhancer runs — Advance does its own research via `find_topic_urls` / `extract_url` / `web_search`
9. Advance bootstrap: if `issue` is present, composes the first user message as `{issue}\n\nPatient: {age}-year-old {gender}.` (falls back to just `issue` if age/gender missing), writes it into `#adv-input`, and calls `handleSend()` so the turn is submitted automatically

### QueryEnhancer (OBSOLETE — no longer invoked from Home)

Home no longer calls `enhanceQuery`. The middleware (`collector-middleware.js#EnhanceQuery`), IPC channel `ENHANCE_QUERY`, and preload `enhanceQuery` remain registered for reversibility. Historical description:
...
### Questionnaire screen → LLM question generation (OBSOLETE — no longer reachable)

Home now routes to Advance. Files, IPC handlers, and this workflow remain for reversibility.
...
### Questionnaire submit → Laboratory screen (OBSOLETE)
...
### Laboratory screen → LLM lab-test generation (OBSOLETE)
...
### Laboratory submit → Pre-doctor Room screen (OBSOLETE)
...
### Pre-doctor Room screen → LLM clarifying-question generation (OBSOLETE)
...
### Pre-doctor Room submit → Doctor screen (OBSOLETE)
...
### Doctor screen → streaming multi-doctor analysis (OBSOLETE — no longer reachable)
...
| **Submit button** | Dark rounded square (`10px`) inside input, right-aligned; white arrow SVG icon; no hover animation. Navigates immediately to Advance — no QueryEnhancer spinner |
| **QueryEnhancer progress stack** | OBSOLETE — home no longer runs QueryEnhancer. CSS for `.qe-toast-stack` may still exist in `app.css` but is unused |
| **Worker snack styles (shared)** | Defined in `worker-snack.css`. No longer linked by home. Still used by the obsolete fanout screens |
...
| **Reasoning level dropdown** | Sits on the **left** of the selects row (`.home-reasoning-wrap`) with a small soft-white `.home-select-label` ("Reasoning level"). Same `.glass-select` style as gender/age. Five options — None / Low / Medium / High / Very High — **Medium selected by default**. On submit `app.js` stashes the value in `sessionStorage['neuroagi:advanceReasoningLevel']`. Advance reads that key and sends it as `reasoningLevel` on every `advanceSend`. There is no Settings overlay or reasoning control on the Advance page |
...
Pastel-gradient chat UI: header with Back to home, `#adv-thread` of user/assistant bubbles, status step pills for tool work, optional `ask_user` form cards, and a bottom composer. **No Settings chip, overlay, or reasoning dropdown** — reasoning is chosen on Home and read from `sessionStorage['neuroagi:advanceReasoningLevel']`.

### Questionnaire + Laboratory screen (`questionnaire.css`) — OBSOLETE

These screens are no longer reachable from Home. Styles remain for reversibility.
...
### Doctor screen (`doctor.css`) — OBSOLETE
