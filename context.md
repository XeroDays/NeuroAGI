...
4. The middleware builds the initial intake prompt via `GenerateQuestionnaireLLMQuery()`. It then runs a **two-stage AGI pipeline** instead of a single LLM call:
   - **Stage 1 — Fanout**: `AskAllWorkerAgis(prompt, JSON_LLM_OPTIONS, { onWorkerSettled, shouldProceedEarly })` in `agi-service.js` issues parallel `chatCompletion` calls to every enabled model from `getActiveModelIds()`. Each worker has a **120s timeout**; timed-out or failed workers are dropped. Early exit: when `shouldProceedEarly()` is true (user clicked merge) and at least one worker succeeded, fanout resolves immediately with settled results only — in-flight slow workers may still finish in the background but are excluded from merge. Each result is shaped uniformly: `{ model, ok, content?, error? }` — a single worker 429/network/timeout failure doesn't abort the others.
