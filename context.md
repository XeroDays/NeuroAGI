...
│   │   └── cookie-middleware.js    # Thin wrapper around model-config-service for IPC calls: GetModelsConfig() → returns [{ name, type, enabled }] for the popup UI; UpdateModelsConfig({ activeModels }) → persists a new activation set via modelConfigService.updateActivation(). Called by the GET_MODELS_CONFIG and UPDATE_MODELS_CONFIG IPC handlers in register.js
│   ├── helpers/
...
│   │   ├── agi-service.js         # Multi-model fanout (parallel worker calls) + master merge. No longer owns static model arrays — both AskAllWorkerAgis and StreamFromAllDoctorAgis call getActiveModels() which delegates to model-config-service at runtime. OPENROUTER_MASTER_MODEL (baidu/cobuddy:free) is still static and not user-configurable. Exports AskAllWorkerAgis(prompt, options?), AskMasterAgi(prompt, options?), StreamFromAllWorkerAgis(prompt, callbacks, options?, modelList?) (modelList=null → falls back to getActiveModels()), StreamFromAllDoctorAgis(prompt, callbacks, options?) — a thin wrapper that calls StreamFromAllWorkerAgis with getActiveModels(), and getActiveModels() for consumers that need the current list. FALLBACK_MODELS is a hardcoded minimal set used only when the config service is not yet initialised
│   │   ├── model-config-service.js # Owns the user-configurable model list. Reads models-catalog.json (project root, [{name, type}] format) and models-state.json (auto-generated, persists { activeModels: string[] }). init() is called once at app startup from main/index.js before registerIpcHandlers(). getModelsWithState() → [{name, type, enabled}] for the popup UI. getActiveModelIds() → full runtime IDs (appends :free for Free type models) of all enabled entries; falls back to the full catalog if the active set is empty. updateActivation(names) → filters against catalog, saves state; never allows a fully empty active set. State file missing on first launch → all models enabled by default
...
models-state.json               # Auto-generated at project root by model-config-service on first run (or when missing). Stores { activeModels: string[] } — which catalog entries are currently toggled on. Git-ignorable; missing file triggers a safe default (all models enabled).
├── preload/
...
2. Click opens the Models popup overlay (`#models-overlay`): calls `window.electronAPI.getModelsConfig()` → IPC `GET_MODELS_CONFIG` → `register.js` → `cookie-middleware.GetModelsConfig()` → `model-config-service.getModelsWithState()` → returns `[{name, type, enabled}]`
3. `app.js` renders one row per model in `#models-list`: model name (monospace), Free/Paid badge, toggle switch. Each toggle mutates the local `modelsState` array in memory without persisting yet
4. `Close` dismisses the popup without saving; `Escape` key and backdrop click also close without saving
5. `Update` button collects all entries where `enabled === true`, calls `window.electronAPI.updateModelsConfig({ activeModels })` → IPC `UPDATE_MODELS_CONFIG` → `cookie-middleware.UpdateModelsConfig()` → `model-config-service.updateActivation(names)` → persists to `models-state.json`. At least one model is always kept active (service guard). Button shows `Saving…` while the IPC call is in flight, then the popup closes
6. On next AGI call, `agi-service.getActiveModels()` reads the updated in-memory cache from `model-config-service.getActiveModelIds()` — no restart needed
...
| **Models popup** | Full-viewport overlay (semi-transparent dark backdrop + 6px blur, `z-index: 200`). Glass card (`rgba(255,255,255,0.13)` + 24px blur, white border, `20px` radius) max-width 540px / 78vh tall. Header: title "Models" + subtitle. Scrollable body: one row per catalog model — monospace name, Free bubble (green tint, `#cdfadb` text) or Paid bubble (blue tint, `#c8e4ff` text), purple toggle switch (same accent `#b48cd2` as questionnaire). Footer: `Close` (dark `#555` bg, white text, `10px` radius) + `Update` (pink `rgba(236,72,153,0.82)` bg, white text, pink glow shadow). Close/Escape/backdrop click dismiss without saving; Update persists + closes |
