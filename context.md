...
│   │   ├── app.js            # Home screen: populates UI, reads the Reasoning level dropdown (#select-reasoning) and stashes the chosen value in sessionStorage['neuroagi:reasoningLevel'], awaits resetUsageTotals() to zero the cost + tokens bubbles for a fresh diagnostic run. On submit, if the issue text is non-empty it runs the QueryEnhancer: shows the circular spinner on the send button (.is-loading), subscribes to onQueryEnhancerProgress (showToast → bottom-right .qe-toast pills), awaits enhanceQuery({issue,gender,age}), and uses the returned enhancedQuery string (falls back to the raw issue on failure/empty) as the issue URL param when navigating to the questionnaire. Empty issue skips the enhancer
│   │   ├── questionnaire.js  # Questionnaire screen: subscribes to onReportCollectionProgress for live status updates, calls startReportCollection on load, hides spinner before renderQuestions, renders per-type controls, on Submit calls submitQuestionnaire IPC, stashes Q&A in sessionStorage['neuroagi:questionnaire'], navigates to laboratory screen
...
│   │   ├── pre-doctor-room.js # Pre-doctor Room screen: reads intake Q&A AND lab Q&A from sessionStorage, calls gotoPreDoctorRoom on load (passing both blocks so the LLM can see what's already known). When the LLM returns zero clarifying questions (empty array — nothing left to ask), shows a brief "No follow-up questions needed. Proceeding…" status and programmatically clicks Submit so the flow continues to the doctor screen without user interaction. Otherwise renders per-type controls with the questionnaire-style delete-X card (duplicated helpers from questionnaire.js — NO lab toggle). On Submit calls submitPreDoctorRoom IPC, stashes pre-doctor Q&A in sessionStorage['neuroagi:preDoctorRoom'], navigates to doctor screen
...
└── shared/
    └── ipc/
        └── channels.js       # IPC channel name constants (mirrored in preload). All channels: PING, ENHANCE_QUERY, QUERY_ENHANCER_PROGRESS, REPORT_COLLECTION_PROGRESS, START_REPORT_COLLECTION, SUBMIT_QUESTIONNAIRE, GOTO_LABORATORY, SUBMIT_LABORATORY, GOTO_PRE_DOCTOR_ROOM, SUBMIT_PRE_DOCTOR_ROOM, START_DOCTOR, DOCTOR_STREAM_DELTA, DOCTOR_STREAM_REASONING_DELTA, DOCTOR_STREAM_DONE, DOCTOR_STREAM_ERROR, GET_USAGE_TOTALS, RESET_USAGE_TOTALS, USAGE_UPDATE, OPEN_DEV_TOOLS, GET_MODELS_CONFIG, UPDATE_MODELS_CONFIG, GET_LOGS, CLEAR_LOGS, LOG_UPDATE
```
...
4. Before navigating, `app.js` also reads the **Reasoning level** dropdown (`#select-reasoning`, default `medium`) and stashes the chosen value in `sessionStorage['neuroagi:reasoningLevel']` so it survives the navigation chain (Home → Questionnaire → Laboratory → Pre-doctor Room → Doctor) — the value is consumed by `doctor.js`, which forwards it on the `startDoctor` IPC payload to `collector-middleware.js#StartDoctor` where `resolveProseOptions(reasoningLevel)` selects the per-call `{ maxTokens, reasoning }` from the `PROSE_LLM_OPTIONS_BY_LEVEL` lookup
5. `app.js` awaits `window.electronAPI.resetUsageTotals()` (IPC `RESET_USAGE_TOTALS` → `usageTracker.resetTotals()`) so the top-right cost + tokens bubbles reset to `USD 0` / `0 tokens` before the new run begins — returning to home via Back links does **not** reset; only the Start button (or Ctrl/Cmd+Enter shortcut) does
6. If the issue text is non-empty, `app.js` runs the **QueryEnhancer** (see next section) before navigating — this is the only LLM/API work the home screen performs. When the issue is empty it skips the enhancer entirely
7. `app.js` builds query string (`?issue=...&gender=...&age=...`) and navigates to `screens/questionnaire/index.html` — the `issue` param is the QueryEnhancer's `enhancedQuery` (original complaint + a `Medication by user:` section) when the enhancer ran, otherwise the raw issue
...
1. `app.js` sets the send button to `.is-loading` (circular spinner overlay, arrow hidden), disables the input + button, and subscribes to `window.electronAPI.onQueryEnhancerProgress` — each `{ message, status }` event becomes a bottom-right `.qe-toast` pill that auto-dismisses after ~3.5s (`status` tints it: active = glass, `done` = green, `error` = red)
...
3. **Stage 1 — detect**: `EnhanceQuery` emits `Scanning your description for medications…`, builds `GenerateMedicineFilterLLMQuery({ issue, gender, age })`, calls `AskMasterAgi(prompt, JSON_LLM_OPTIONS)` (single master call — no worker fanout), and runs the result through the shared `parseJsonArray`. Entries are normalized to `{ name, mg, timing }` and any with no `name` are dropped. If the list is empty (no meds mentioned, or parse failed), it emits `No medications detected` and returns `{ ok: true, enhancedQuery: <original issue> }`
4. **Stage 2 — lookup**: for each detected medicine (sequentially so each gets its own toast) it emits `Looking up <name>…` and calls `webSearch.search('active ingredient of the medication "<name>"[ <mg>][ taken <timing>]', { includeAnswer: true, maxResults: 3 })` (Tavily) — the reported dosage/timing are folded into the query so the search surfaces the clinically plausible drug for ambiguous/mistyped names. A per-medicine lookup failure is tolerated (that medicine just gets an empty evidence set)
5. **Stage 3 — rewrite**: emits `Enhancing your query with medication details…`, builds `GenerateEnhancedQueryLLMQuery({ issue, medicines, searchResults })`, calls `AskMasterAgi(prompt, JSON_LLM_OPTIONS)`, and cleans the text via `stripFences()` (the output is plain text, not JSON). The prompt resolves each medicine's **active ingredient name** and applies context-aware disambiguation (uses reported dosage/form/timing to correct likely-misspelled brand names — e.g. "Topagen 50 mg every night" → `topiramate`). If the call fails or returns empty it **falls back** to `buildFallbackEnhancedQuery(issue, medicines)` (original issue + a `Medication by user:` section built from the detected names/mg/timing). Emits `Found N medication(s)` and returns `{ ok: true, enhancedQuery }`
6. **Graceful degradation**: the whole body is wrapped in try/catch — any throw (no master model starred, LLM error, etc.) emits `Could not analyze medications — continuing` and returns `{ ok: true, enhancedQuery: <original issue> }`, so the user is never blocked on the home screen
7. `app.js` uses `res.enhancedQuery` (falling back to the raw issue when it is empty/missing) as the `issue` URL param, unsubscribes from the progress channel, and navigates to the questionnaire. `questionnaire.js` is unchanged — it reads that `issue` from the URL for both the visible summary and `startReportCollection({ issue, gender, age })`, so the medication section flows into `GenerateQuestionnaireLLMQuery` as part of the presenting issue with no questionnaire-specific code
8. **Usage**: because `resetUsageTotals()` runs before the enhancer, the two master calls + the Tavily lookups all count toward the run's cost/token totals and update the home usage bubbles live via the `USAGE_UPDATE` broadcast (Tavily calls add tokens/cost only if OpenRouter-style usage is reported; Tavily itself does not feed `usageTracker`). The medication context lives only inside the `issue` string — the laboratory / pre-doctor / doctor prompts are unchanged. Trade-off: because the enhanced `issue` is also what the questionnaire summary line shows, the `Medication by user:` section is visible there too
...
3. `app.js` renders one row per model in Free/Paid tabs: grey star button (left), model name, Free/Paid badge, optional latency/throughput/price bubbles, optional custom label pills (semicolon-separated `labels` string from catalog — each segment becomes a `.models-label-badge` with shared cyan fill `rgb(22 144 173 / 88%)` + white text, no border), toggle switch. Star click sets exclusive master (`isMaster` on one model across both tabs; click starred star again to deselect). When a **new** model is starred, `app.js#setMasterModel` also turns that model's worker toggle **on** and turns the **previous** master's worker toggle **off** so changing master stops fanout requests to the replaced model (still requires `Update` to persist). Other toggles mutate local `modelsState` without persisting yet
4. `Close` dismisses the popup without saving; `Escape` key and backdrop click also close without saving
5. `Update` sends `{ activeModels, masterModel }` via `updateModelsConfig` → `model-config-service.updateState()` → persists to userData `models-state.json`. Server-side safeguard: when `masterModel` changes, the previous master is removed from `activeModels` and the new master is added. Button shows `Saving…` while in flight, then popup closes
6. On next AGI call, worker fanout reads `getActiveModelIds()` and master merge reads `getMasterModelRuntimeId()` — no restart needed. If no master is starred, `AskMasterAgi` throws and the collector returns `{ ok: false }`

### Questionnaire screen → LLM question generation

1. `questionnaire.js` on `DOMContentLoaded` reads `issue`, `gender`, `age` from URL params, fills the summary, and shows a `Loading questions…` status. It subscribes to `onReportCollectionProgress` so `#q-status .q-status-label` updates live during collection (worker fanout / master merge stages)
2. It calls `window.electronAPI.startReportCollection({ issue, gender, age })` → IPC `START_REPORT_COLLECTION`
3. Main process: `register.js` invokes `StartReportcollection(payload, event.sender)` in `collector-middleware.js`, which emits `REPORT_COLLECTION_PROGRESS { message, status }` to the renderer at each stage
4. The middleware builds the initial intake prompt via `GenerateQuestionnaireLLMQuery()`. It then runs a **two-stage AGI pipeline** instead of a single LLM call:
   - **Stage 1 — Fanout**: `AskAllWorkerAgis(prompt, JSON_LLM_OPTIONS, { onWorkerSettled })` in `agi-service.js` issues `chatCompletion(messages, model, { timeoutMs: 120000, … })` calls in parallel (`Promise.allSettled`) to every enabled model from `getActiveModelIds()`. Each worker has a **120s timeout**; timed-out or failed workers are dropped. Each result is shaped uniformly: `{ model, ok, content?, error? }` — a single worker 429/network/timeout failure doesn't abort the others.
...
4. Main process: `register.js` invokes `GotoLaboratory()` in `collector-middleware.js`, which mirrors `StartReportcollection`'s **two-stage AGI pipeline** but with a different initial prompt:
   ...
   - **Fanout**: identical to questionnaire — `AskAllWorkerAgis(prompt, JSON_LLM_OPTIONS)` across enabled models from `getActiveModelIds()` with `Promise.allSettled`
   ...
   - **Master merge**: reuses the existing `GenerateMergeQuestionnaireLLMQuery({ issue, gender, age, questionnaireSets: parsedSets })` prompt (it is generic over question arrays of the standard schema, no laboratory-specific variant needed). `AskMasterAgi(mergePrompt, JSON_LLM_OPTIONS)` against the user-starred master model produces the final consolidated list of result-input questions. The same `JSON_LLM_OPTIONS = { maxTokens: 4096, reasoning: { effort: "none" } }` flow that protects the questionnaire path also protects the lab path
...
3. Main process: `register.js` invokes `GotoPreDoctorRoom()` in `collector-middleware.js`, which mirrors `GotoLaboratory`'s **two-stage AGI pipeline** with a different initial prompt:
   ...
   - **Fanout / per-worker tiered parse / all-fail guard / master merge / fallback**: byte-identical to `GotoLaboratory` — `AskAllWorkerAgis(prompt, JSON_LLM_OPTIONS)` across enabled models from `getActiveModelIds()` with `Promise.allSettled`, tiered `parseJsonArray` (strict → normalize → jsonrepair), drop failures, throw `All worker models failed or returned unparsable JSON` if zero workers succeeded, then reuse `GenerateMergeQuestionnaireLLMQuery({ issue, gender, age, questionnaireSets: parsedSets })` against `AskMasterAgi(mergePrompt, JSON_LLM_OPTIONS)` to consolidate, with a `pickBestWorkerSet(parsedSets)` fallback when the master parse throws or returns empty content. Logs are tagged `[collector/predoc]` (`worker ${model}`, `master merge`, `final pre-doctor`)
4. On success the middleware returns `{ ok: true, issue, gender, age, questions }`. If `questions` is a non-empty array, `pre-doctor-room.js` renders the returned clarifying questions through the **questionnaire-style** card helpers — same `buildCard` with the top-right delete-X button, same `collectSurviving` / `collectAnswers`, NO lab-style "I have this report" toggle (only `laboratory.js` renders that). The helpers are duplicated verbatim into `pre-doctor-room.js`, matching the existing duplication pattern between `questionnaire.js` and `laboratory.js`. If `questions` is empty (the model decided no further clarification is needed after intake + lab), the renderer does **not** show an error — it displays a brief centered status (`No follow-up questions needed. Proceeding…`) and programmatically triggers the Submit button, which runs the normal `submitPreDoctorRoom` IPC, stashes `{ questions: [], answers: [] }` into `sessionStorage['neuroagi:preDoctorRoom']`, and navigates to the doctor screen
...
| **QueryEnhancer toasts** | `.qe-toast-stack` fixed bottom-right (`bottom: 1rem; right: 1rem; z-index: 50`, column, right-aligned, `pointer-events: none`). Each `.qe-toast` is a thin rounded glass pill (`999px`, blurred translucent bg, white `0.78rem` text, `qe-toast-in` slide/fade keyframes) injected by `app.js#showToast` per progress event and auto-removed after ~3.5s (adds `.qe-toast--leaving` for the exit transition). `status` tints it: active = default glass, `done` = soft green, `error` = soft red |
